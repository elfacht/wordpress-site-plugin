...
function foundation_pagination( $p = 2 ) {
	...
	if ( $paged > $p + 1 ) p_link( 1, 'Erste Seite' );
	...
	if ( $paged < $max_page - $p ) p_link( $max_page, 'Letzte Seite' );
}
function p_link( $i, $title = '' ) {
	if ( $title == '' ) $title = "Seite {$i}";
	echo "<li><a href='", esc_html( get_pagenum_link( $i ) ), "' title='{$title}'><span class='inner'>{$i}</span></a></li> ";
}
